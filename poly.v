...
Definition partition {X : Type}
                    ...
                    (l : list X)
                    : list X * list X :=
(filter (fun m => test m) l, filter (fun m => (negb (test m))) l).
...
(* Pick something more general, I originally went with
map on rev with one single element, then just map on rev
and finally i opted to just distribute map on two lists. *)
(*Also if you don't use something, and you use rewrite with
it on another function, then you will be left with that unused
variable in goal.*)
Lemma map_app_helper : forall (X Y : Type) (f : X -> Y)
(l1 : list X) (l2 : list X),
...
Example test_fold_lengthl : fold_length [4;7;0] = 3.
Proof. reflexivity. Qed.
